...
def to_chart(songr,filename,num):
    ...
    #fig.show()
    fig.write_image(filename+"/fig"+str(num)+".jpeg") #save chart into file
...
column = tracks["Tempo"] #normalizing tempo values
...
#print(tracks)
index=1
for track in tracks: #create radar chart for each song using song's features
    if(track[-1] == 1):
